...
@section('nav')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">{{trans('language.brands')}}</li>
    </ol>
@endsection
@section("buttons")
        <a class="btn btn-success col-md-1 " href="{{url("/admin/brands/create")}}">
            {{trans("language.add")}}
        </a>
@endsection
@section('thead')
...
@section('tbody')
    @foreach($items as $item)
        <tr class="text-center">
            ...
            <td>
                    @includeIf("admin.components.buttons.edit" , ["href" => "brands/$item->id/edit"])
                    @includeIf("admin.components.buttons.delete",["message" => "($item->name)" ,  "action" => url("admin/brands/$item->id")])
            </td>
        </tr>
    @endforeach
@endsection
...
@section("filters")
    <form method="get" action="{{url("/admin/brands/")}}">
