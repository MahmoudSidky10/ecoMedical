...
@section('page-title',trans('language.governorates'))
@section('root' , "governorates")
@section('nav')
    <ol class="breadcrumb">
...
@endsection
@section("buttons")
        <a class="btn btn-success col-md-1" href="{{url("/admin/governorates/create")}}">
            {{trans("language.add")}}
        </a>
@endsection
@section('thead')
    ...
    <th>{{trans('language.delivery_cost')}}</th>
...
@section('tbody')
    @foreach($items as $item)
        <tr>
            ...
            <td>
                    @includeIf("admin.components.buttons.edit" , ["href" => "governorates/$item->id/edit"])
                    @includeIf("admin.components.buttons.delete",["message" => "($item->name)" ,  "action" => url("admin/governorates/$item->id")])
            </td>
        </tr>
    @endforeach
@endsection
...
@section("filters")
    <form method="get" action="{{url("/admin/governorates/")}}">
